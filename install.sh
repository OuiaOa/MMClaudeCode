...
if (( node_major < 22 )); then
  echo "Node $node_major detected — mmclaude needs v22 or newer for the private Claude Code runner. Please upgrade: https://nodejs.org/"; exit 1
fi

# ------------------------------------------------------ Optional existing Claude detection
claude_bin="$(command -v claude || true)"
if [[ -z "$claude_bin" ]]; then
  echo "Claude Code is not installed globally; mmclaude setup will install a private runner."
fi
