#!/usr/bin/env node
/**
 * Tests for bin/dsv4f-scrub.mjs. The whole point of this module is "never delete something
...
} from './bin/dsv4f-scrub.mjs';

const require = createRequire(import.meta.url);
const DatabaseSync = (() => { try { return require('node:sqlite').DatabaseSync; } catch { return null; } })();

const SCRATCH = fs.mkdtempSync(path.join(os.tmpdir(), 'dsv4f-scrub-test-'));
...
console.log('\n\x1b[1mdsv4f-scrub tests\x1b[0m\n');
...
console.log('\n\x1b[1mperformCliRemoval\x1b[0m');
{
  ...
  const dataDir = path.join(dir, 'dsv4f-data');
  ...
  const r3 = performCliRemoval({
    binaryPath: null, dataDir: path.join(dir, 'dsv4f-data-2'), credentialsPath, credentialsExist: false,
