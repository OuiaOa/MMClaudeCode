...
// The port in config.json is a preferred port, not an exclusive claim.  This small
// manager lets separately-installed Claude shims coexist without making either one
// rewrite the other application's settings.
...
function nestedPort(value) {
  for (const key of ['port', 'listenPort']) {
    const port = validPort(value?.[key]);
    if (port) return port;
  }
  for (const key of ['server', 'listen', 'gateway', 'http']) {
    const port = validPort(value?.[key]?.port);
    if (port) return port;
  }
  return 0;
}
...
function scanDir(root, depth, ownPaths, out, seen) {
  ...
  for (const entry of entries) {
    ...
    const data = jsonFile(full);
    const port = nestedPort(data);
...
function discoveredReservations({ app, configDir, dataDir, scanRoots = [] }) {
  const out = [];
  const ownPaths = [configDir, dataDir];
  const roots = [
...
/** Pick and persist the first usable port at or after the preferred candidates. */
...
export async function healthAt(port, endpoint, timeoutMs = 1200) {
  try {
    const response = await fetch(`http://127.0.0.1:${port}${endpoint}`, { signal: AbortSignal.timeout(timeoutMs) });
    return response.ok;
  } catch { return false; }
}

/** Keep an isolated/rerouted Claude profile in step with an automatically shifted port. */
export function syncLoopbackProfile(settingsPath, port) {
  const data = jsonFile(settingsPath);
