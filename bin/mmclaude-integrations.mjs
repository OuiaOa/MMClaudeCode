#!/usr/bin/env node
/**
 * Small, provider-neutral capability layer for the bundled Claude profile.
 *
 * This intentionally discovers tools instead of installing them. Defuddle, Agent Reach,
 * Composio, and codebase-memory-mcp each have their own update/auth/security lifecycle. A
 * shim should make them easy to use when present without turning a normal Claude launch into
 * a package manager or a surprise account login.
 */
...
/** Return a PATH-resolved executable or null, without throwing or printing. */
export function findExecutable(names, { env = process.env } = {}) {
  for (const name of (Array.isArray(names) ? names : [names])) {
    ...
    } catch { /* tool lookup is best effort */ }
  }
  // A few Windows installs expose a command only through an explicit .cmd/.exe name.
  if (WIN) {
    for (const name of (Array.isArray(names) ? names : [names])) {
...
/**
 * Create a separate, shim-owned MCP config when codebase-memory-mcp is installed.
 * `--mcp-config` is passed only when this file exists, so a normal install has no new
 * dependency and a user's own ~/.claude.json or project .mcp.json is never overwritten.
 */
export function syncAutoMcpConfig(profileDir, { capabilities = discoverCapabilities() } = {}) {
  ...
  const config = {
    mcpServers: {
      'codebase-memory-mcp': { command: capabilities.codebaseMemory, args: [] },
    },
  };
...
export function capabilityContext({ cwd = process.cwd(), profileDir, capabilities = discoverCapabilities() } = {}) {
  const lines = [
    ...
    'For any normal http(s) webpage, use `mmclaude web <url>` first: it uses Defuddle when installed and a clean-reader fallback otherwise; do not send ads/navigation into the context.',
    ...
    'Parallel work is allowed only for independent tasks. Respect the shim traffic queue: prefer one main lane plus at most one background/helper lane, keep helper prompts short, and do not fan out speculative agents.',
...
export function capabilityReport({ capabilities = discoverCapabilities() } = {}) {
  const rows = [
    ['Defuddle web extraction', capabilities.defuddle, 'mmclaude web <url>'],
...
if (import.meta.url === `file://${process.argv[1]?.replace(/\\/g, '/')}`) {
  const profileDir = process.env.MMCLAUDE_PROFILE_DIR || process.env.CLAUDE_CONFIG_DIR;
