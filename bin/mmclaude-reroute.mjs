...
export function buildRerouteEnv({ port, sentinel }) {
  return {
    ...
    // Token Plan pauses can last until the rolling five-hour interval ends; keep the Claude
    // request alive while the shim waits, while the shim still enforces its own upstream timeouts.
...
function installQualityHooks(settings, rootDir) {
  const session = path.join(rootDir, 'bin', 'mmclaude-quality-session.mjs');
  const check = path.join(rootDir, 'bin', 'mmclaude-quality-check.mjs');
