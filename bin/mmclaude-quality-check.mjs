...
const lockDir = path.join(os.tmpdir(), 'mmclaude-quality');
...
try {
  ...
  if (Date.now() - stat.mtimeMs < 10 * 60_000) {
    console.log(JSON.stringify({ hookSpecificOutput: { hookEventName: 'PostToolUse', additionalContext: `MMClaudeCode quality gate already running for ${cwd}; its result will be reported on a later turn.` } }));
    process.exit(0);
  }
...
if (!checks.length) {
  console.log(JSON.stringify({ hookSpecificOutput: { hookEventName: 'PostToolUse', additionalContext: `MMClaudeCode quality gate: no existing local check command discovered after ${relFile || 'the change'}; load mmclaude-code-quality for the appropriate focused check.` } }));
  process.exit(0);
}
const results = [];
let failed = false;
...
const msg = `MMClaudeCode quality gate: ${status} after ${relFile || 'the change'}\n${results.join('\n')}${failed ? '\nLoad mmclaude-code-quality and inspect the failure before continuing.' : ''}`;
