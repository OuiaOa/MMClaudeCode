...
export function privateClaudeCandidates(dataDir, platform = process.platform) {
  ...
  return platform === 'win32'
    ? [path.join(bin, 'claude.cmd'), path.join(bin, 'claude.exe')]
...
export function installPrivateClaude({ dataDir, platform = process.platform, npm = platform === 'win32' ? 'npm.cmd' : 'npm', exec = spawnSync } = {}) {
  ...
  const result = exec(npm, ['install', '--prefix', root, '--no-package-lock', '--no-fund', '--no-audit', `${PACKAGE}@latest`], {
    stdio: 'inherit',
    windowsHide: true,
  });
  if (result?.status !== 0) {
    throw new Error(`could not install the private Claude Code runner (npm exit ${result?.status ?? 'unknown'})`);
  }
  ...
  try {
    fs.writeFileSync(path.join(root, 'install.json'), JSON.stringify({ package: PACKAGE, installedAt: new Date().toISOString() }, null, 2) + '\n', { mode: 0o600 });
  } catch {}
  return found;
}
