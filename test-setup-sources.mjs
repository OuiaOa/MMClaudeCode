#!/usr/bin/env node
/**
 * Tests for bin/dsv4f-setup-sources.mjs — the orchestration that turns the picker's
 * leave/copy/move/remove choices into real dsv4f-import + dsv4f-scrub calls. This is glue
 * code over already-unit-tested building blocks (dsv4f-sources, dsv4f-scrub,
 * dsv4f-opencode-convert); what matters here is that it wires them together with the RIGHT
...
import { applySourceDispositions } from './bin/dsv4f-setup-sources.mjs';

const require = createRequire(import.meta.url);
const DatabaseSync = (() => { try { return require('node:sqlite').DatabaseSync; } catch { return null; } })();

const ROOT = path.resolve(import.meta.dirname);
const SCRATCH = fs.mkdtempSync(path.join(os.tmpdir(), 'dsv4f-setup-sources-test-'));
...
console.log('\n\x1b[1mdsv4f-setup-sources tests\x1b[0m\n');
...
console.log('\x1b[1mcopy: imports but never scrubs\x1b[0m');
{
  ...
  await applySourceDispositions({
    ...
    importEnv: { HOME: claudeHome, CLAUDE_DSV4F_PROFILE: profileDir },
    importStdio: 'pipe', log: quiet, errorLog: quiet,
  });

  check('session imported into the dsv4f profile',
...
console.log('\n\x1b[1mshared-transcript independence: desktop move must NOT touch cli\'s transcripts\x1b[0m');
{
  ...
  check('the transcript was still imported into dsv4f (desktop\'s "remove" still copies first)',
    fs.existsSync(path.join(profileDir, 'projects', 'C--Users-test', 'sessionA.jsonl')));
}
