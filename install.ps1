# mmclaude installer - Windows 11 (PowerShell)
#
# There is no systemd here, so the shim is started on demand by `mmclaude run` instead of by a
...
#   -Bundle           copy Claude Code's binary into the mmclaude install, so the resulting
...
$dest = if ($env:MMCLAUDE_HOME) { $env:MMCLAUDE_HOME } else { Join-Path $HOME ".local\share\mmclaude" }
...
if ($nodeMajor -lt 22) {
    throw "Node $nodeVersion detected -- mmclaude needs v22 or newer for the private Claude Code runner. Please upgrade: https://nodejs.org/"
}
...
if (-not $claudeBin) {
    Write-Host "Claude Code is not installed globally; mmclaude setup will install a private runner." -ForegroundColor Yellow
}
...
if ($src -ne $dest -or $Update) {
    ...
    # Skills (Claude Code Skills) -- auto-discovered under the mmclaude profile. Copy
...
# Copies claude into the mmclaude install so the resolver can prefer it. Makes the mmclaude
...
# Legacy aliases are removed separately so the hash contains one unambiguous mmclaude entry.
$entries = @{ "mmclaude" = "bin\mmclaude.mjs";
              "mmclaude-usage" = "bin\mmclaude-usage"; "mmclaude-import" = "bin\mmclaude-import" }
...
Write-Host "Next:  mmclaude setup"
